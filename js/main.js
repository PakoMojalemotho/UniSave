...
(function () {
  ...
  /* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
     MODULE: Payments
     Persists business subscription state so the
     dashboard, nav, and other pages can surface
     plan status without re-fetching.
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
  var Payments = {
    KEY: 'unisave_subscription',

    /** @returns {object|null} active subscription or null */
    get: function () {
      try { return JSON.parse(localStorage.getItem(Payments.KEY) || 'null'); }
      catch (e) { return null; }
    },

    /** @param {object} sub */
    set: function (sub) {
      localStorage.setItem(Payments.KEY, JSON.stringify(sub));
    },

    clear: function () {
      localStorage.removeItem(Payments.KEY);
    },

    /** Returns true if a paid plan is active */
    isActive: function () {
      var sub = Payments.get();
      if (!sub || sub.status !== 'active') return false;
      return sub.price === 0 || Date.now() < sub.renewsAt;
    },

    /** Human-readable renewal date string */
    renewalLabel: function () {
      var sub = Payments.get();
      if (!sub || sub.price === 0) return 'No renewal — always free';
      var d = new Date(sub.renewsAt);
  ...
  function initAuthNav() {
    ...
    // Check for active subscription and show plan badge
    var sub = Payments.get();
    var planBadge = '';
    if (sub && sub.plan && sub.plan !== 'Starter') {
      planBadge = '<span style="display:inline-block;padding:2px 7px;border-radius:999px;background:var(--amber);color:#fff;font-size:0.58rem;font-weight:800;text-transform:uppercase;letter-spacing:0.06em;margin-left:4px;">' + sub.plan + '</span>';
    }

    var subLine = sub
      ? '<div style="padding:8px 16px;font-size:0.74rem;color:var(--text-muted);border-bottom:1px solid var(--border);background:var(--surface);">📦 ' + (sub.plan || 'Free') + ' Plan' + (sub.price > 0 ? ' · renews ' + Payments.renewalLabel() : ' · Always free') + '</div>'
      : '<div style="padding:8px 16px;font-size:0.74rem;color:var(--text-muted);border-bottom:1px solid var(--border);background:var(--surface);"><a href="businesses.html" style="color:var(--amber);font-weight:700;text-decoration:none;">+ List your business</a></div>';
    ...
    actions.innerHTML =
      ...
      '</div>' +
      '<div id="nav-user-dd" style="position:fixed;top:var(--nav-h);right:1.5rem;background:var(--card);border:1px solid var(--border);border-radius:var(--radius-lg);box-shadow:var(--shadow-lg);min-width:200px;z-index:1100;overflow:hidden;opacity:0;pointer-events:none;transform:translateY(-8px);transition:opacity 0.2s,transform 0.2s;">' +
        subLine +
        ...
        '<a href="payment.html" style="display:flex;align-items:center;gap:10px;padding:12px 16px;font-size:0.84rem;font-weight:600;color:var(--text);border-bottom:1px solid var(--border);">💳 Manage plan</a>' +
